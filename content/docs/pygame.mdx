...
&gt; In `pygame` coordinates starts from 
```math
[0, 0]
``` at top left and 
```math
x
``` goes from left to right, while 
```math
y
...
Here we are changing snail frame at 
```math
5
``` fps using `snail_index`. And are changing snail's x-coordinate at `SNAIL_SPEED` to make it infinitely loop from end to start.
