...
**Regression**: predicts a numeric value. Eg: Chances of rain fall. 
```math
33\%
```
...
a
```&#125;
- If apply *one-vs-rest* for pairs, Classes may overlap i.e. from both 
```math
a
``` and 
```math
b
```. And may not fall under any category (middle).
