...
```math
transactions,
```AB$'s balance should increment at the same time. During failure, both balances should return to normal.
- **Consistency:**
	- Guarantees transactions transition the database between valid states.
	- Eg: Ones balance shouldn't be negative. Hence a 100
```math
transaction from an account with balance 50
``` failed due to consistency violation.
- **Isolation:**
...
5. **Foreign Key**:
	...
	- By referencing primary key of another table 
```math
R1
``` inside current table 
```math
R2
```.
	- 
...
```math
A \rightarrow B
``` where is B is a subset of A. Eg: `A -> A`.
	2. **Non-Trivial FD**: 
```math
A \rightarrow B
``` where B isn't a subset of A. i.e. 
```math
A \cap B = \phi
``` .
...
Pattern 1,2,3,4,5,6s balance should be reduce and 
```math
B
```'s balance should increment at the same time. During failure, both balances should return to normal.
- **Consistency:**
	- Guarantees transactions transition the database between valid states.
	- Eg: Ones balance shouldn't be negative. Hence a 100
```math
