...
&gt; *Result(s, a)*: state reached by performing action 
```math
a
``` on state 
```math
s
```.
&gt; *cost(s, a)*: cost of performing action 
```math
a
``` on state 
```math
...
```math
d
``` is depth of shallowed goal node.
- Space complexity: 
```math
O(b^d)
```; Stores all nodes at lowest level.
...
```math
m
``` is maximum depth of search tree.
- SC : 
```math
O(b^m)
```; stores longest depth in memory.
...
```math
h(n)
```*: gives estimation on cost of getting from node 
```math
n
``` to goal state.
...
h(n)
``` is evaluated first.
- Algorithm : Using priority queue based on 
```math
h(n)
```. start root. end goal.
- Advance:
	- find solution without much exploration
	- less memory comparison to 
```math
A*
``` etc.
- Disadvantage:
...
- Consistency (monotonicity) : for every successor 
```math
n'
``` of 
```math
...
- Eg: Sudoku.
- *Crypt Arithmetic Problem:* Find values of variables that satisfy a sum operation. Domain 
```math
\in [0,9]
```. Condition: values must be unique. Observation: carry can only be 
```math
1
``` or 
```math
0
```. Extra digit at sum can only be 
```math
1
```.
...
\alpha \ge \beta
```.
- Maximum node updates 
```math
\alpha
``` from children, and similarly minimum updates 
```math
\beta
```.
...
```math
p
``` is called *hypothesis* (or premise) and q is called the *conclusion*.
	- 
```math
q \to p
``` converse
...
```math
\forall_x \; P(x)
``` i.e. for all values of x, P(x) is true.
...
```math
\exists_x \; P(x)
``` i.e. for at least one value of x, P(x) is true.
...
1. **Universal instantiation / Universal specialization  / Universal elimination**
	...
	- The UI rule state that we can infer any sentence 
```math
P(c)
``` by substituting a ground term 
```math
c
``` (a constant within a domain x) from 
```math
\forall_x \; P(x)
...
2. **Existential instantiation**
	- "there exists a person who likes pizza" means "
```math
c
``` likes pizza".
	- This rule states that one can infer 
```math
P(c)
``` from formula 
```math
...
3. **Existential generalization / Existential introduction**
	...
	- If there is some element 
```math
c
``` in the universe of discourse which has a property 
```math
P
```, then we can infer that there exists something in the universe which has a property 
```math
P
```.
...
```math
P(c)
``` is true for any arbitrary element 
```math
c
``` in the universe of discourse, then we can have a conclusion as 
```math
\forall_x \; P(x)
```
...
Ex: "Charlie is a dog" =&gt; logical representation 
```math
Dog(Charlie)
```
...
6. Range constraints - logical expression 
```math
6 \lt i \lt 10
```
...
```math
P(A/B)
``` is *Posterior* (prob. of A when B is true)
&gt; 
```math
P(B/A)
``` is *Likelihood* (prob. of evidence)
&gt; 
```math
P(A)
``` is *Prior probability* (prob. of hypothesis)
&gt; 
```math
P(B)
``` is *Marginal probability* (prob. of evidence)
...
**Degree of Truth:** Generalizes the classical “indicator” function (0 or 1) to a continuous scale 
```math
[0,1]
```.
**Vagueness & Uncertainty:** Captures how strongly a given crisp input 
```math
x
...
```math
X
``` is **membership space** (universe of discourse)
#### Key Features
...
\{x\mid \mu_A(x)=0.5\}
``` |
| **Boundary**                 | Region between support and core  | 
```math
\{x\mid 0<\mu_A(x)<1\}
``` |
