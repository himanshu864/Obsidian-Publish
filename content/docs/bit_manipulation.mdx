...
To convert a bit representation 
```math
b_k\ldots b_2b_1b_0
...
The `int` type in C++ is a signed type, so an `int` variable can contain any integer between 
```math
-2^{31}
``` and 
```math
...
In C++, an `unsigned int` variable can contain any integer between 
```math
0
``` and 
```math
...
In an unsigned representation, the next number after 
```math
2^n-1
``` is 
```math
...
```math
n
``` is a power of 2 if there is only one set bit in its binary representation.
...
```math
n
``` is a power of 4 if it is a power of 2 and it's previous number is divisible by 3.
