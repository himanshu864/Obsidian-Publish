...
Data communication system has _five components_ -
...
- First data is _encapsulated_ from Application Layer -> Physical Layer
...
- Finally data is _de-encapsulated_ to extract the data.
...
- Ensures _error-free_ data transfer between two devices on same network
- Divides data into _frames_
...
- Divides data into _packets_, assigns IP address, chooses best path
- Eg: IPv4, routers
...
- Divides data into _segments_, provides error recovery, flow control and retransmission of lost packets.
- Eg: TCP, UDP
...
- Up to _1600 Gbps_: higher bandwidth, less signal attenuation, no noise problem, no corrosion, light weight, greater immunity to tapping
...
- Then message is transferred as a complete unit and forwarded using _Store and Forward_ mechanism.
...
- Each independent entity is called as _datagram_.
- Path is not fixed
...
$$T_p = \frac{Distance}{Propagation Speed}$$
...
_Vulnerable Time_: During which there is a possibility of collision, if another frame is send.
...
In DLL, Error control is commonly implemented through the _Automatic Repeat Request_ (ARQ) process, where detected errors trigger the retransmission of specific frames to maintain data integrity.
...
Each frame has its own countdown timer and sends _negative_ acknowledgements if lost.
...
- _D: Minimize delay_
- _T: Maximize throughput_
- _R: Maximize reliability_
- _C: Minimize cost_
...
- _offset_ (13 bits) shows relative position of fragment w.r.t whole datagram.
...
2. _D-bit (Do-not fragment bit)_: tells not to fragment this datagram, if set (1).
3. _M-bit (More fragment bit)_: tells this is not the last fragment, if set.
...
- To remedy that, sender included it's MAC and IP address of both sender and receiver and _broadcasts_ it to the local network.
- The receiver identifies himself, and sends his MAC address to sender's IP as _response_.
...
- Represented in decimal and has 4 _octets_ (x.x.x.x)
...
- 192.168.1.255 is reserved for the chatty _Broadcast Address_. He everything to everyone.

So that's makes it a total of **254** different hosts!

### Classes of IPv4 Address (_Classful_)
...
### Masks in IPv4 Addressing (_Subnet Mask_)
...
### Default Masks for IPv4 Classes (_Subnet classes_)
...
- Routers are the devices that perform this function by maintaining and updating _routing tables_, which list the routes to various network destinations.
...
- **Basic Principle:**
  ...
  - _Triggered Update_ A node sends its two-column routing table to its neighbors anytime there is a change in its routing table. (update or failure)
...
**Goal:** Shape traffic _before it enters_ the network to limit bursts and smooth peaks.
...
**Goal:** Hosts adjust send rates based on feedback (losses, delays, or explicit marks) received from the network _after it happens._
...
- **Multiple Recipients** - emails can be send to many people at once using _Carbon Copy (CC)_ and _Blind Carbon Copy (BCC)_.
